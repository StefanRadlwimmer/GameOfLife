...
string clErrorstring(cl_int err) {
  switch (err) {
...
void handle_clerror(cl_int err) {
  if (err != CL_SUCCESS){
    cerr << "OpenCL Error: " << clErrorstring(err) << string(".") << endl;
    exit(EXIT_FAILURE);
  }
}
...
int main(int argc, char **argv){
  ...
  for(unsigned int i=0;i<numPlatforms;i++) {
	...
	for(unsigned int j=0;j<numDevices;j++) {
	  ...
	  char name[255];
	  err = clGetDeviceInfo(deviceId, CL_DEVICE_NAME, sizeof(char) * 255, &name, NULL);
	  handle_clerror(err);
	  print_dname("  Name:"); cout << name << endl;
      ...
      handle_clerror(err);
      print_dname("  Max work item dimensions");cout<<maxWorkItemDimensions<<endl;
      ...
      handle_clerror(err);
	  print_dname("  Max work item sizes");
